...
### Intersections and recursions are based on GPS points representing centre points of the enclosures for now; will change these to polygons of the enclosures and/or coordinates of the specific feeding locations.
...
# Read in the coordinates for the feeding enclosures
park_coords <- read.csv("FI_GPS coordinates.csv") ### Change these to polygons of the enclosures or coordinates of the feeding cages

# Convert enclosure coordinates into Lambert projection
coordinates(park_coords) <- c("Longitude", "Latitude")
...
# Set buffers
park_buffer <- st_buffer(geom_locs, dist = 50) # 'dist' is in meters, setting a buffer for 50m
...
projection(presence_sf)
projection(park_buffer)
class(presence_sf$geometry)

intersect_dat <- presence_sf %>% mutate(
  intersection = as.character(st_intersects(geometry, park_buffer)),
...
locs <- read.csv("FI_GPS coordinates.csv") ### Change these to polygons of the enclosures or coordinates of the feeding cages
